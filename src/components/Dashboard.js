...
function SortableItem({
    ...
    return (
      ...
      >
            ...
            <Menu as="div" className="relative">
                ...
                <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
                    <Menu.Items className="absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white dark:bg-slate-800 shadow-lg ring-1 ring-black dark:ring-slate-700 ring-opacity-5 focus:outline-none">
                        <div className="py-1">
                            <Menu.Item><button onClick={() => onEdit(block)} className="group flex w-full items-center px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"><BiPencil className="mr-3 h-5 w-5"/> Sửa</button></Menu.Item>
                            <Menu.Item><button onClick={() => onDelete(id)} className="group flex w-full items-center px-4 py-2 text-sm text-red-500 hover:bg-red-500/10"><BiTrash className="mr-3 h-5 w-5"/> Xóa</button></Menu.Item>
                        </div>
                    </Menu.Items>
                </Transition>
            </Menu>
        </div>
        <button {...attributes} {...listeners} className="cursor-grab p-2 text-slate-500 dark:text-slate-400 mb-2 self-center">
            <BiMenu size={22} />
        </button>
        ...
        <div className="flex flex-col items-center mt-3 space-y-2">
            <div className="flex items-center text-slate-500 dark:text-slate-400 text-sm"><BiBarChart className="mr-1.5"/> <span className="font-mono">{clickCount}</span></div>
            <Switch checked={block.isEnabled} onChange={() => onToggleStatus(id, !block.isEnabled)} className={clsx("relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-slate-800", block.isEnabled ? "bg-blue-600" : "bg-slate-300 dark:bg-slate-600")}><span className={clsx("inline-block h-4 w-4 transform rounded-full bg-white transition-transform", block.isEnabled ? "translate-x-6" : "translate-x-1")}/></Switch>
        </div>
      </motion.div>
    );
  }
  ...
  return (
    <motion.div ref={setNodeRef} style={style} variants={fadeInUp} layoutId={id} className={clsx(itemClasses, "p-3")}>
        <div className="flex items-center gap-3 w-full min-w-0">
            <div className="flex-shrink-0 flex items-center">
                ...
                <button {...attributes} {...listeners} className="cursor-grab p-2 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ml-2"><BiMenu size={22}/></button>
            </div>
            <div className="flex-shrink-0 text-3xl text-slate-500 dark:text-slate-400 w-10 h-10 flex items-center justify-center">{icon}</div>
            <div className="flex-grow min-w-0">
                <p className="font-semibold text-slate-800 dark:text-slate-100 truncate">{title || "Chưa có tiêu đề"}</p>
                <p className="text-sm text-slate-500 dark:text-slate-400 truncate">{subtitle || "URL không hợp lệ"}</p>
            </div>
            <div className="flex-shrink-0 flex items-center ml-4 space-x-1">
                <div className="hidden sm:flex items-center text-slate-500 dark:text-slate-400 text-sm mr-2"><BiBarChart className="mr-1.5"/> <span className="font-mono">{clickCount}</span></div>
                <Switch checked={block.isEnabled} onChange={() => onToggleStatus(id, !block.isEnabled)} className={clsx("relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-slate-800", block.isEnabled ? "bg-blue-600" : "bg-slate-300 dark:bg-slate-600")}><span className={clsx("inline-block h-4 w-4 transform rounded-full bg-white transition-transform", block.isEnabled ? "translate-x-6" : "translate-x-1")}/></Switch>
                <Menu as="div" className="relative">
                    <Menu.Button className="p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full"><BiDotsVerticalRounded size={20}/></Menu.Button>
                    <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
                        <Menu.Items className="absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white dark:bg-slate-800 shadow-lg ring-1 ring-black dark:ring-slate-700 ring-opacity-5 focus:outline-none">
                            <div className="py-1">
                                <Menu.Item><button onClick={() => onEdit(block)} className="group flex w-full items-center px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"><BiPencil className="mr-3 h-5 w-5"/> Sửa</button></Menu.Item>
...
const SkeletonLoader = () => (
    <div className="space-y-8">
        ...
        <div className="space-y-3"><div className="h-16 rounded-xl bg-slate-200 dark:bg-slate-800 animate-pulse"></div><div className="h-16 rounded-xl bg-slate-200 dark:bg-slate-800 animate-pulse"></div><div className="h-16 rounded-xl bg-slate-200 dark:bg-slate-800 animate-pulse"></div></div>
    </div>
);
...
const QuickStats = ({ blocks, stats }) => {
  ...
  return (
    <motion.div variants={fadeInUp} className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700"><h3 className="text-sm font-medium text-slate-500 dark:text-slate-400">Tổng số khối</h3><p className="mt-1 text-3xl font-semibold tracking-tight text-slate-900 dark:text-white">{blocks.length}</p></div>
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700"><h3 className="text-sm font-medium text-slate-500 dark:text-slate-400">Tổng lượt click</h3><p className="mt-1 text-3xl font-semibold tracking-tight text-slate-900 dark:text-white">{totalClicks}</p></div>
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700"><h3 className="text-sm font-medium text-slate-500 dark:text-slate-400">Hiệu quả nhất</h3>{topBlock ? (<p className="mt-1 text-lg font-semibold text-blue-600 dark:text-blue-400 truncate">{JSON.parse(topBlock.data).title || "Khối không tên"}</p>) : (<p className="mt-1 text-lg text-slate-400">Chưa có dữ liệu</p>)}</div>
    </motion.div>
  );
};
...
const OnboardingChecklist = ({ profile, blocks }) => {
    const tasks = [
      ...
      { text: "Tùy chỉnh giao diện", completed: profile?.background !== "dynamic-default-bg" },
    ];
        ...
        <ul className="space-y-3">
          {tasks.map((task, index) => (
            <li key={index} className="flex items-center text-sm">
              {task.completed ? <FaCheckCircle className="text-green-500 mr-3 flex-shrink-0" /> : <FaRegCircle className="text-slate-400 dark:text-slate-500 mr-3 flex-shrink-0" />}
              <span className={clsx({ "text-slate-500 dark:text-slate-400 line-through": task.completed, "text-slate-800 dark:text-slate-200": !task.completed })}>{task.text}</span>
            </li>
          ))}
        </ul>
      </Card>
...
const AddBlockModal = ({ isOpen, onClose, onSelect }) => {
    const renderMenuItem = (item) => (
        <button key={item.label} onClick={() => onSelect(item)} className="group flex w-full flex-col items-center justify-center text-center p-4 rounded-lg bg-slate-50 dark:bg-slate-700/50 hover:bg-blue-100 dark:hover:bg-blue-900/20 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-200">
            <div className="text-2xl mb-2 text-slate-600 dark:text-slate-300 group-hover:text-blue-600 dark:group-hover:text-blue-400">{item.icon}</div>
                    ...
                    <div className="flex min-h-full items-center justify-center p-4 text-center">
                        <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                            <Dialog.Panel className="w-full max-w-3xl transform overflow-hidden rounded-2xl bg-slate-100 dark:bg-slate-800 p-6 text-left align-middle shadow-xl transition-all">
                                <Dialog.Title as="h3" className="text-lg font-bold leading-6 text-slate-900 dark:text-slate-100 flex justify-between items-center">
                                    Thêm khối mới
                                    <button onClick={onClose} className="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700"><BiX size={20}/></button>
                                </Dialog.Title>
                                <div className="mt-4 space-y-6">
                                    <div>
                                        <p className="px-1 py-1.5 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Ủng hộ</p>
                                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mt-2">{monetizationLinks.map(renderMenuItem)}</div>
                                    </div>
                                    <div>
                                        <p className="px-1 py-1.5 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Mạng xã hội</p>
                                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mt-2">{socialLinks.map(renderMenuItem)}</div>
                                    </div>
                                    <div>
                                        <p className="px-1 py-1.5 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Nội dung nhúng</p>
                                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mt-2">{embedContent.map(renderMenuItem)}</div>
                                    </div>
                                </div>
                            </Dialog.Panel>
                        </Transition.Child>
...
export default function Dashboard({ profile: initialProfile, onProfileUpdate }) {
  ...
  const handleFileUpload = async (file, setterFn) => {
    ...
    if (imageUrl) { setterFn(imageUrl); }
  };
  const onSocialImageChange = async (event) => {
    const file = event.target.files[0];
    if (!file) return;
    setIsUploadingSocial(true);
    await handleFileUpload(file, (url) => setSettings((prev) => ({ ...prev, socialImage: url })));
  ...
  const onBgImageChange = async (event) => {
    ...
    await handleFileUpload(file, (url) => setSettings((prev) => ({ ...prev, background: url })));
  ...
  const handleProfileSave = async () => {
    ...
    try {
      const payload = { ...initialProfile, ...settings, userId: currentUser.uid };
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/profiles`, payload);
      onProfileUpdate(response.data);
      toast.success("Đã lưu cài đặt thành công!");
    } catch (err) {
      const errorMessage = err.response?.data?.message || "Lưu cài đặt thất bại.";
  ...
  const handleOpenModalToEdit = (block) => {
    ...
    const label = embedContent.find((item) => item.type === block.type)?.label || allLinks.find((item) => { try { return item.placeholder.includes(new URL(JSON.parse(block.data)?.url).hostname); } catch { return false; } })?.label || "Link tùy chỉnh";
  ...
  const handleDeleteBlock = (blockId) => {
    setConfirmState({
      ...
      message: "Bạn có chắc chắn muốn xóa khối này không? Hành động này không thể hoàn tác.",
      onConfirm: async () => {
        try {
          await axios.delete(`${process.env.REACT_APP_API_URL}/api/blocks/${blockId}`);
          setBlocks((prev) => prev.filter((b) => b.id !== blockId));
          toast.success("Đã xóa khối.");
        } catch (err) { toast.error("Xóa khối thất bại."); }
      },
    });
  };
  const handleSaveBlock = async (rawData) => {
    ...
    if (blockType === "link" && linkInfo && linkInfo.prefix && finalData.url && !finalData.url.startsWith("http")) {
      finalData.url = linkInfo.prefix + finalData.url;
    }
    const blockData = { type: blockType, data: JSON.stringify(finalData), isEnabled: editingBlock ? editingBlock.isEnabled : true };
    try {
      let response;
      if (editingBlock) {
        response = await axios.put(`${process.env.REACT_APP_API_URL}/api/blocks/${editingBlock.id}`, blockData);
        setBlocks((prev) => prev.map((b) => (b.id === editingBlock.id ? response.data : b)));
      } else {
        response = await axios.post(`${process.env.REACT_APP_API_URL}/api/blocks/${currentUser.uid}`, blockData);
        setBlocks((prev) => [...prev, response.data]);
      }
      onProfileUpdate({ blocks: [...blocks.filter((b) => b.id !== response.data.id), response.data] });
      toast.success("Đã lưu khối thành công!");
    } catch (err) { toast.error("Lưu khối thất bại."); }
    setEditingBlock(null);
  };
...
const handleToggleStatus = async (blockId, isEnabled) => {
  ...
  const handleBulkAction = async (action) => {
    ...
    if (action === "delete") {
      ...
      setBlocks((prev) => prev.map((b) => selectedBlocks.includes(b.id) ? { ...b, isEnabled } : b));
    }
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/api/blocks/bulk-action`, { action, blockIds: selectedBlocks });
      toast.success(`Đã ${actionVerb[action]} ${selectedBlocks.length} khối đã chọn!`);
      setSelectedBlocks([]);
    } catch (error) {
    ...
    <>
      ...
      >
        <div className="lg:col-span-2 space-y-6">
            <motion.div variants={fadeInUp}>
                <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-800 dark:text-slate-100 break-words">Chào mừng trở lại, {currentUser.displayName || "bạn"}!</h1>
            ...
            <AnimatePresence mode="wait">
                <motion.div key={activeTab}>
                    {activeTab === 0 && <OnboardingChecklist profile={initialProfile} blocks={blocks} />}
                </motion.div>
            </AnimatePresence>
            ...
            <Card className="p-4 sm:p-6">
                ...
                <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-2 p-2 bg-slate-100 dark:bg-slate-700/50 rounded-lg border border-slate-200 dark:border-slate-600">
                    <div className="flex items-center flex-grow min-w-0"><BiLinkAlt className="text-slate-400 mx-2 hidden sm:block flex-shrink-0"/><a href={bioLink} target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 font-mono flex-grow truncate text-center sm:text-left min-w-0">{bioLink}</a></div>
                        ...
                        <Menu as="div" className="relative">
                            ...
                            <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
                                <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white dark:bg-slate-800 shadow-lg ring-1 ring-black dark:ring-slate-600 ring-opacity-5 focus:outline-none">
                                    <div className="py-1">
                                        <Menu.Item>{({active})=>(<a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(bioLink)}`} target="_blank" rel="noopener noreferrer" className={`${active?"bg-slate-100 dark:bg-slate-700":""} text-slate-700 dark:text-slate-300 group flex items-center w-full px-4 py-2 text-sm transition-colors`}><FaFacebook className="mr-3"/> Facebook</a>)}</Menu.Item>
                                        <Menu.Item>{({active})=>(<a href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(bioLink)}&text=${encodeURIComponent("Xem trang của tôi!")}`} target="_blank" rel="noopener noreferrer" className={`${active?"bg-slate-100 dark:bg-slate-700":""} text-slate-700 dark:text-slate-300 group flex items-center w-full px-4 py-2 text-sm transition-colors`}><FaTwitter className="mr-3"/> Twitter / X</a>)}</Menu.Item>
                                    </div>
                                </Menu.Items>
                            </Transition>
                        </Menu>
              ...
              <Card>
                <Tab.List className="flex gap-x-1 sm:gap-x-2 p-2 bg-slate-100 dark:bg-slate-700/50 rounded-t-2xl overflow-x-auto">
                    {mainTabs.map((tab, index) => (
                         <Tab key={tab.name} className={({ selected }) => clsx("w-full whitespace-nowrap flex items-center justify-center gap-x-2 py-2.5 px-4 font-medium text-sm focus:outline-none transition-all duration-300 rounded-lg", selected ? "bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 shadow" : "text-slate-500 hover:bg-slate-200/70 dark:text-slate-400 dark:hover:bg-slate-600/50")}>
                  ...
                  <Tab.Panel className="focus:outline-none">
                    <div className="p-4 sm:p-6">
                        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 flex-shrink-0">Các khối nội dung</h2>
                            <div className="flex flex-col sm:flex-row items-center gap-2 w-full">
                                <div className="relative flex-grow w-full"><BiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none"/><input type="text" placeholder="Tìm kiếm khối..." value={searchTerm} onChange={(e)=>setSearchTerm(e.target.value)} className="pl-9 w-full border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900/50 text-slate-900 dark:text-slate-200 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition-colors"/></div>
                                <div className="flex items-center gap-2 w-full sm:w-auto self-end">
                                    <div className="flex items-center bg-slate-100 dark:bg-slate-700 p-1 rounded-lg"><button onClick={()=>setLayout("list")} className={clsx("p-1.5 rounded-md transition-colors",layout==="list"?"bg-white dark:bg-slate-800 shadow text-blue-600":"text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100")}><BiListUl size={20}/></button><button onClick={()=>setLayout("grid")} className={clsx("p-1.5 rounded-md transition-colors",layout==="grid"?"bg-white dark:bg-slate-800 shadow text-blue-600":"text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100")}><BiGridAlt size={20}/></button></div>
                                    <motion.button whileTap={{scale:0.95}} onClick={() => setIsAddBlockModalOpen(true)} className="w-full justify-center inline-flex items-center gap-x-1.5 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-slate-900"><BiPlus className="h-5 w-5 -ml-1"/> Thêm khối</motion.button>
                                </div>
                            </div>
                        </div>
                    </div>
      ...
      <motion.button onClick={() => setIsPreviewOpen(true)} className="lg:hidden fixed bottom-6 right-6 z-40 bg-blue-600 text-white rounded-full p-4 shadow-lg flex items-center justify-center" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
        <BiShow size={24} />
      </motion.button>
      
      <AnimatePresence>
        {selectedBlocks.length > 0 && (
          <motion.div initial={{y:100,opacity:0}} animate={{y:0,opacity:1}} exit={{y:100,opacity:0}} transition={{type:"spring",stiffness:300,damping:30}} className="fixed bottom-4 left-1/2 -translate-x-1/2 w-auto max-w-[calc(100%-2rem)] bg-white dark:bg-slate-800 shadow-2xl rounded-xl border border-slate-200 dark:border-slate-700 p-2 z-40 flex items-center gap-1 sm:gap-2">
      ...
      <AddBlockModal isOpen={isAddBlockModalOpen} onClose={() => setIsAddBlockModalOpen(false)} onSelect={handleOpenModalToAdd}/>
      <EditBlockModal isOpen={isEditBlockModalOpen} onClose={() => setIsEditBlockModalOpen(false)} onSave={handleSaveBlock} block={editingBlock} modalInfo={modalInfo}/>
      
      <AnimatePresence>
        {isQrModalOpen && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=>setIsQrModalOpen(false)} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      ...
      <Transition appear show={isPreviewOpen} as={Fragment}>
        <Dialog as="div" className="relative z-50 lg:hidden" onClose={() => setIsPreviewOpen(false)}>
            ...
            <div className="fixed inset-0 overflow-y-auto"><div className="flex min-h-full items-center justify-center p-4 text-center"><Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95"><Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-transparent text-left align-middle transition-all"><MobilePreview profile={{ ...initialProfile, ...settings }} blocks={blocks}/><button onClick={() => setIsPreviewOpen(false)} className="mt-4 w-full bg-white/20 text-white py-2 rounded-lg backdrop-blur-md">Đóng</button></Dialog.Panel></Transition.Child></div></div>
        </Dialog>
    </Transition>

      <ConfirmModal isOpen={confirmState.isOpen} onClose={() => setConfirmState({ ...confirmState, isOpen: false })} onConfirm={confirmState.onConfirm} title={confirmState.title}>{confirmState.message}</ConfirmModal>
    </>
  );
}